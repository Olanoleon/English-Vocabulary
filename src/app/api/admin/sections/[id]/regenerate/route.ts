...
- Mix of three styles (distribute evenly):
  1. "multiple_choice" (definition): "What is the definition of 'word'?" with 4 Spanish definition options (1 correct, 3 plausible distractors)
  2. "multiple_choice" (reverse): "Which English word means 'definición en español'?" with 4 English word options from the vocabulary list (1 correct, 3 other words)
  3. "fill_blank": A standalone generic sentence where the vocabulary word fits naturally. Set correct_answer to the word.
...
- phonetics questions must use a MIX of these three styles (vary them):
...
Each question object: { "type": string, "prompt": string, "correctAnswer": string|null, "options": [{"optionText": string, "isCorrect": boolean}] }`;
...
export async function POST(
  ...
  try {
    ...
    const content = await callOpenAI(
      REGEN_PROMPT,
      `Vocabulary words:\n${vocabSummary}\n\nGenerate exactly ${practiceCount} practice questions and exactly ${testCount} test questions.\nReturn the JSON object now.`
    );
